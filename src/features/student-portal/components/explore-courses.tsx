  ...
  return (
    <article className="group flex h-full flex-col overflow-hidden rounded-2xl border border-[#E2E0EF] bg-white p-4 transition hover:border-[#C3C0FF] hover:shadow-[0_4px_12px_rgba(79,70,229,0.04)]">
      <div className="relative h-40 overflow-hidden rounded-xl bg-[#F0ECF9]">
        <Image src={course.img || lessonFallback} alt={course.title} fill sizes="(max-width: 767px) 100vw, 33vw" className="object-cover transition-transform duration-500 group-hover:scale-[1.03]" />
      ...
      <div className="mt-4 flex flex-1 flex-col">
        <span className="w-fit rounded-lg bg-[#4F46E5]/5 px-2.5 py-1 text-xs font-bold text-[#3525CD]">{course.subject_name || "كورس تعليمي"}</span>
        <h3 className="mt-3 line-clamp-2 min-h-14 text-lg font-black leading-7 text-[#1B1B24]">{course.title}</h3>
        <Link href={`/explore/teachers/${teacher.slug}`} className="mt-1 inline-flex w-fit items-center gap-1.5 text-xs font-bold text-[#777587] hover:text-[#3525CD]"><UserRound className="size-3.5" />{teacher.name}</Link>
        ...
        <div className="mt-auto flex items-end justify-between gap-3 pt-5">
          ...
          <Link href={href} className={`inline-flex h-10 items-center gap-2 rounded-xl px-4 text-xs font-black transition-colors ${enrolled ? "bg-emerald-600 text-white hover:bg-emerald-700" : "bg-[#4F46E5] text-white hover:bg-[#3525CD]"}`}>{enrolled ? "فتح الكورس" : "عرض الكورس"}<ArrowLeft className="size-4" /></Link>
        </div>
      </div>
    </article>
  );
...
function TeacherCard({ teacher, courseCount }: { teacher: PublicTeacherDto; courseCount: number }) {
  ...
  return (
    <article className="flex min-h-44 items-center gap-5 rounded-2xl border border-[#E2E0EF] bg-white p-5">
      <div className="relative size-20 shrink-0 overflow-hidden rounded-full border border-[#E2E0EF] bg-[#F0ECF9] sm:size-24">
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-1 line-clamp-1 text-xs font-bold text-[#4F46E5]">{subjects.join("، ") || "مدرس على منصة علمني"}</p>
        ...
        <Link href={`/explore/teachers/${teacher.slug}`} className="mt-4 inline-flex h-9 items-center gap-2 rounded-lg border border-[#C3C0FF] px-4 text-xs font-black text-[#3525CD] hover:bg-[#F5F2FF]">عرض المدرس<ArrowLeft className="size-3.5" /></Link>
      </div>
    </article>
  );
}
    ...
    <StudentPortalShell user={user} active="discover">
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <span className="mb-3 inline-flex items-center gap-2 rounded-lg bg-[#F0ECF9] px-3 py-1.5 text-xs font-black text-[#3525CD]"><Compass className="size-4" />استكشف</span>
            <h1 className="flex items-center gap-3 text-3xl font-black leading-tight sm:text-4xl"><Search className="size-8 text-[#4F46E5]" />اكتشف كورسات جديدة</h1>
            <p className="mt-2 text-sm leading-6 text-[#777587]">اختر المدرس والكورس المناسبين لمرحلتك الدراسية.</p>
          </div>
          {(profileGrade || profileStream) && <span className="inline-flex w-fit items-center gap-2 rounded-full border border-[#E2DFFF] bg-[#4F46E5]/5 px-4 py-2 text-xs font-black text-[#3525CD]"><span className="size-2 rounded-full bg-[#4F46E5]" />{[profileGrade?.name, profileStream?.name].filter(Boolean).join(" - ")}</span>}
        </header>

        <section className="mt-7" aria-label="بحث وفلاتر الكورسات">
          <label className="relative block max-w-4xl">
            ...
            <input value={search} onChange={(event) => changeFilter(setSearch, event.target.value)} placeholder="ابحث عن مادة أو مدرس أو كورس..." className="h-14 w-full rounded-lg border border-[#C7C4D8] bg-white pe-12 ps-4 text-sm outline-none transition focus:border-[#4F46E5] focus:ring-2 focus:ring-[#4F46E5]/10 sm:text-base" />
          </label>
          {!!suggestedSearches.length && <div className="mt-3 flex flex-wrap items-center gap-2 text-xs text-[#777587]"><span>مقترحات:</span>{suggestedSearches.map((item) => <button key={item} type="button" onClick={() => changeFilter(setSearch, item)} className="cursor-pointer rounded-lg bg-[#F0ECF9] px-2.5 py-1 font-bold text-[#464555] hover:text-[#3525CD]">{item}</button>)}</div>}

          <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <label><span className="sr-only">الصف الدراسي</span><select value={grade} onChange={(event) => changeFilter(setGrade, event.target.value)} className="h-11 w-full cursor-pointer rounded-lg border border-[#E2E0EF] bg-white px-3 text-sm font-bold outline-none focus:border-[#4F46E5]"><option value="all">كل الصفوف</option>{grades.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
            <label><span className="sr-only">الشعبة</span><select value={stream} onChange={(event) => changeFilter(setStream, event.target.value)} className="h-11 w-full cursor-pointer rounded-lg border border-[#E2E0EF] bg-white px-3 text-sm font-bold outline-none focus:border-[#4F46E5]"><option value="all">كل الشعب</option>{streams.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
            <label><span className="sr-only">المدرس</span><select value={teacher} onChange={(event) => changeFilter(setTeacher, event.target.value)} className="h-11 w-full cursor-pointer rounded-lg border border-[#E2E0EF] bg-white px-3 text-sm font-bold outline-none focus:border-[#4F46E5]"><option value="all">كل المدرسين</option>{teachers.map((item) => <option key={item.slug} value={item.slug}>{item.name}</option>)}</select></label>
            <label className="relative"><span className="sr-only">ترتيب النتائج</span><SlidersHorizontal className="pointer-events-none absolute end-3 top-1/2 size-4 -translate-y-1/2 text-[#777587]" /><select value={sort} onChange={(event) => { setSort(event.target.value as CatalogSort); setPage(1); }} className="h-11 w-full cursor-pointer appearance-none rounded-lg border border-[#E2E0EF] bg-white pe-10 ps-3 text-sm font-bold outline-none focus:border-[#4F46E5]"><option value="newest">الأحدث</option><option value="price-low">السعر: الأقل أولاً</option><option value="price-high">السعر: الأعلى أولاً</option></select></label>
          </div>

          {!!subjects.length && <div className="mt-5 flex gap-2 overflow-x-auto pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden" aria-label="المواد"><button type="button" onClick={() => changeFilter(setSubject, "all")} className={`h-10 shrink-0 rounded-full px-5 text-xs font-black ${subject === "all" ? "bg-[#4F46E5] text-white" : "border border-[#C7C4D8] bg-white text-[#464555]"}`}>كل المواد</button>{subjects.map((item) => <button key={item.id} type="button" onClick={() => changeFilter(setSubject, item.name)} className={`h-10 shrink-0 rounded-full px-5 text-xs font-black ${subject === item.name ? "bg-[#4F46E5] text-white" : "border border-[#C7C4D8] bg-white text-[#464555] hover:bg-[#F5F2FF]"}`}>{item.name}</button>)}</div>}
        </section>
        ...
        {!hasFilters && !!recommended.length && (
          <section className="mt-10" aria-labelledby="recommended-heading">
            <div className="mb-5 flex flex-wrap items-end justify-between gap-3"><div><h2 id="recommended-heading" className="flex items-center gap-2 text-2xl font-black"><Target className="size-6 text-[#4F46E5]" />مناسب لك</h2><p className="mt-1 text-xs text-[#777587]">{profile ? "بناءً على المرحلة والشعبة المحفوظتين في ملفك." : "اختيارات من أحدث الكورسات المنشورة على المنصة."}</p></div><button type="button" onClick={() => document.getElementById("all-courses")?.scrollIntoView({ behavior: "smooth" })} className="inline-flex cursor-pointer items-center gap-1 text-xs font-black text-[#3525CD] hover:underline">عرض الكل<ArrowLeft className="size-3.5" /></button></div>
        ...
        <section id="all-courses" className="scroll-mt-24 pt-10" aria-labelledby="all-courses-heading">
          ...
          {visibleCourses.length ? (
            <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">{visibleCourses.map((entry) => <CourseCard key={entry.course.id} entry={entry} enrolled={enrolledCourseIds.includes(entry.course.id)} />)}</div>
          ) : (
            <div className="flex min-h-64 flex-col items-center justify-center rounded-2xl border border-[#E2E0EF] bg-white px-4 text-center"><Search className="mb-4 size-9 text-[#C7C4D8]" /><h3 className="text-lg font-black">لا توجد كورسات مطابقة</h3><p className="mt-2 text-sm text-[#777587]">جرّب تغيير البحث أو اختيار تصنيف آخر.</p>{hasFilters && <button type="button" onClick={resetFilters} className="mt-4 cursor-pointer text-sm font-black text-[#3525CD] hover:underline">مسح الفلاتر</button>}</div>
          )}

          {totalPages > 1 && <nav className="mt-8 flex items-center justify-center gap-2" aria-label="صفحات الكورسات"><button type="button" onClick={() => setPage((current) => Math.max(1, current - 1))} disabled={page === 1} aria-label="الصفحة السابقة" className="flex size-10 items-center justify-center rounded-lg border border-[#E2E0EF] bg-white disabled:cursor-not-allowed disabled:opacity-40"><ChevronRight className="size-4" /></button>{Array.from({ length: totalPages }, (_, index) => index + 1).map((item) => <button key={item} type="button" onClick={() => setPage(item)} aria-current={page === item ? "page" : undefined} className={`size-10 rounded-lg text-sm font-black ${page === item ? "bg-[#4F46E5] text-white" : "border border-[#E2E0EF] bg-white"}`}>{item}</button>)}<button type="button" onClick={() => setPage((current) => Math.min(totalPages, current + 1))} disabled={page === totalPages} aria-label="الصفحة التالية" className="flex size-10 items-center justify-center rounded-lg border border-[#E2E0EF] bg-white disabled:cursor-not-allowed disabled:opacity-40"><ChevronLeft className="size-4" /></button></nav>}
        </section>

        {!!teachers.length && (
          <section className="pt-12" aria-labelledby="featured-teachers-heading">
            <div className="mb-5 flex items-center justify-between"><h2 id="featured-teachers-heading" className="text-2xl font-black">أبرز المدرسين</h2><Link href="/browse-teachers" className="inline-flex items-center gap-1 text-xs font-black text-[#3525CD] hover:underline">كل المدرسين<ArrowLeft className="size-3.5" /></Link></div>
