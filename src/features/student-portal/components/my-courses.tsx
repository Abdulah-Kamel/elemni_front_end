...
function CourseRow({ enrollment }: { enrollment: EnrollmentDto }) {
  ...
  return (
    <article className="group flex flex-col overflow-hidden rounded-2xl border border-[#E2E0EF] bg-white transition hover:border-[#C3C0FF] hover:shadow-[0_4px_12px_rgba(79,70,229,0.04)] md:min-h-56 md:flex-row-reverse">
      <div className="relative aspect-video shrink-0 overflow-hidden bg-[#F0ECF9] md:aspect-auto md:w-60">
        <Image
      ...
      <div className="flex min-w-0 flex-1 flex-col p-5 sm:p-6">
        <div className="flex flex-wrap items-center gap-2">
          <span className="rounded-full bg-[#4F46E5]/5 px-3 py-1 text-xs font-bold text-[#3525CD]">{course.subject_name || "كورس تعليمي"}</span>
          <span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-bold text-emerald-700">اشتراك نشط</span>
        </div>

        <h2 className="mt-3 text-xl font-black leading-8 text-[#1B1B24] transition-colors group-hover:text-[#3525CD] sm:text-2xl">{course.title}</h2>
        ...
        <div className="mt-5 flex flex-wrap gap-x-5 gap-y-2 border-t border-[#E2E0EF] pt-4 text-xs font-bold text-[#777587]">
          ...
          <span className="inline-flex items-center gap-1.5"><CalendarClock className="size-4 text-[#4F46E5]" />متاح حتى {formatDate(enrollment.expires_at)}</span>
        </div>

        <Link href={`/courses/${course.id}`} className="mt-5 inline-flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-[#4F46E5] px-6 text-sm font-black text-white transition-colors hover:bg-[#3525CD] sm:w-fit">
      ...
      {loading ? (
        ...
        <div className="mx-auto flex min-h-[70vh] max-w-xl items-center px-4">
          <div role="alert" className="w-full rounded-2xl border border-red-200 bg-red-50 p-6 text-center text-sm font-bold text-red-700">
            ...
            <p>{error}</p>
            <button type="button" onClick={() => void loadCourses()} className="mt-4 cursor-pointer text-[#3525CD] hover:underline">إعادة المحاولة</button>
          </div>
        </div>
      ) : (
        <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
          <header className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <span className="mb-3 inline-flex items-center gap-2 rounded-lg bg-[#F0ECF9] px-3 py-1.5 text-xs font-black text-[#3525CD]"><GraduationCap className="size-4" />مكتبة الطالب</span>
            ...
            <Link href="/explore" className="inline-flex h-11 w-fit items-center gap-2 rounded-xl border border-[#C3C0FF] bg-white px-5 text-sm font-black text-[#3525CD] transition hover:bg-[#F5F2FF]"><Compass className="size-4" />استكشف كورسات جديدة</Link>
          </header>

          {!!enrollments.length && (
            <section className="mt-8 grid grid-cols-3 gap-2 sm:gap-3" aria-label="ملخص الكورسات">
              {[
                ...
                <article key={label} className="flex min-h-24 flex-col justify-center gap-2 rounded-2xl border border-[#E2E0EF] bg-white p-3 sm:flex-row sm:items-center sm:gap-4 sm:p-5">
                  <span className="flex size-9 shrink-0 items-center justify-center rounded-xl bg-[#F0ECF9] text-[#4F46E5] sm:size-11"><Icon className="size-4 sm:size-5" /></span>
          ...
          {!!enrollments.length && (
            <section className="mt-8 border-y border-[#E2E0EF] py-4" aria-label="فلترة الكورسات">
              <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_220px_180px]">
                <label className="relative block">
                  ...
                  <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="ابحث باسم الكورس أو المادة..." className="h-11 w-full rounded-lg border border-[#E2E0EF] bg-white pe-11 ps-4 text-sm outline-none transition focus:border-[#4F46E5] focus:ring-2 focus:ring-[#4F46E5]/10" />
                </label>
                ...
                <label className="relative block">
                  ...
                  <select value={subject} onChange={(event) => setSubject(event.target.value)} className="h-11 w-full cursor-pointer appearance-none rounded-lg border border-[#E2E0EF] bg-white pe-10 ps-4 text-sm font-bold outline-none transition focus:border-[#4F46E5] focus:ring-2 focus:ring-[#4F46E5]/10">
                ...
                <label>
                  ...
                  <select value={sort} onChange={(event) => setSort(event.target.value as CourseSort)} className="h-11 w-full cursor-pointer rounded-lg border border-[#E2E0EF] bg-white px-3 text-sm font-bold outline-none transition focus:border-[#4F46E5] focus:ring-2 focus:ring-[#4F46E5]/10">
          ...
          <section className="mt-8" aria-labelledby="courses-heading">
            ...
            {visibleCourses.length ? (
              <div className="space-y-5">{visibleCourses.map((enrollment) => <CourseRow key={enrollment.id} enrollment={enrollment} />)}</div>
            ) : enrollments.length ? (
              <div className="flex min-h-64 flex-col items-center justify-center rounded-2xl border border-[#E2E0EF] bg-white px-4 text-center">
                ...
                {hasFilters && <button type="button" onClick={() => { setSearch(""); setSubject("all"); }} className="mt-4 cursor-pointer text-sm font-black text-[#3525CD] hover:underline">مسح الفلاتر</button>}
              </div>
            ) : (
              <div className="flex min-h-80 flex-col items-center justify-center rounded-2xl border border-[#E2E0EF] bg-white px-4 text-center">
                <span className="flex size-14 items-center justify-center rounded-2xl bg-[#F0ECF9] text-[#4F46E5]"><BookOpen className="size-7" /></span>
                ...
                <Link href="/explore" className="mt-6 inline-flex h-11 items-center gap-2 rounded-xl bg-[#4F46E5] px-6 text-sm font-black text-white hover:bg-[#3525CD]">استكشف الكورسات<ArrowLeft className="size-4" /></Link>
              </div>
            )}
          </section>
        </div>
