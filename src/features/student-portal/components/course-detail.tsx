...
function TeacherAvatar({ name, image }: { name: string; image: string | null }) {
  ...
  return (
    <span className="flex size-11 items-center justify-center rounded-full bg-[#E2DFFF] text-sm font-black text-[#3525CD]">
      {name.slice(0, 1)}
    </span>
  );
}
...
function LessonRow({
  ...
  return (
    <div className="border-b border-[#E2E0EF] last:border-b-0">
      <button
        type="button"
        onClick={onToggle}
        aria-expanded={expanded}
        className="flex min-h-20 w-full cursor-pointer items-center gap-3 px-1 py-4 text-start transition-colors hover:bg-[#F5F2FF] sm:px-3"
      >
        <span className="flex size-10 shrink-0 items-center justify-center rounded-full bg-[#F0ECF9] text-[#4F46E5]">
          {hasVideo ? <PlayCircle className="size-5" /> : hasDocument ? <FileText className="size-5" /> : <BookOpen className="size-5" />}
        </span>
        <span className="min-w-0 flex-1">
          <strong className="block text-sm font-black leading-6 text-[#1B1B24] sm:text-base">{lesson.title}</strong>
        ...
        <ChevronDown className={`size-5 shrink-0 text-[#777587] transition-transform ${expanded ? "rotate-180" : ""}`} />
      </button>

      {expanded && (
        <div className="space-y-2 bg-[#FCFBFF] px-4 py-4 sm:ps-16">
          ...
          {lesson.items.length ? lesson.items.map((item) => {
            ...
            if (item.bunny_stream_embed_url) {
              return (
                <button key={item.id} type="button" onClick={() => onPlay(item, lesson)} className={`flex w-full cursor-pointer items-center gap-3 rounded-lg border px-3 py-3 text-start transition ${isActive ? "border-[#4F46E5] bg-[#F0ECF9]" : "border-[#E2E0EF] bg-white hover:border-[#C3C0FF]"}`}>
                  <ItemIcon className="size-5 shrink-0 text-[#4F46E5]" />
                  ...
                  <Play className="size-4 shrink-0 fill-current text-[#4F46E5]" />
                </button>
              );
            }
            ...
            if (documentUrl) {
              return (
                <a key={item.id} href={documentUrl} target="_blank" rel="noreferrer" className="flex items-center gap-3 rounded-lg border border-[#E2E0EF] bg-white px-3 py-3 text-start transition hover:border-[#C3C0FF]">
            ...
            return (
              <div key={item.id} className="flex items-center gap-3 rounded-lg border border-[#E2E0EF] bg-white px-3 py-3 text-start">
                ...
                <span className="text-[11px] font-bold text-[#A6A3B5]">غير متاح حالياً</span>
              </div>
            );
...
export default function CourseDetail({
  ...
  const loadCourse = useCallback(async () => {
    ...
    setDetail(await courseResponse.json());
  ...
  const playVideo = (item: PublicItemDto, lesson: PublicLessonDto) => {
    ...
    window.setTimeout(() => document.getElementById("course-player")?.scrollIntoView({ behavior: "smooth", block: "start" }), 0);
  };

  const startCourse = () => {
    const firstVideoLesson = lessons.find((lesson) => lesson.items.some((item) => item.bunny_stream_embed_url));
    ...
    <StudentPortalShell user={user} active={enrolled ? "courses" : "discover"}>
      {loading ? (
        ...
        <div className="mx-auto flex min-h-[70vh] max-w-xl items-center px-4">
          <div role="alert" className="w-full rounded-2xl border border-red-200 bg-red-50 p-6 text-center text-sm font-bold text-red-700">
            ...
            <div className="mt-5 flex justify-center gap-4">
              <button type="button" onClick={() => void loadCourse()} className="inline-flex cursor-pointer items-center gap-2 text-[#3525CD] hover:underline"><RotateCcw className="size-4" />إعادة المحاولة</button>
        ...
        <div className="mx-auto max-w-7xl px-4 py-7 sm:px-6 sm:py-8 lg:px-8">
          <Link href={enrolled ? "/my-courses" : "/explore"} className="mb-5 inline-flex items-center gap-2 text-sm font-bold text-[#777587] transition-colors hover:text-[#3525CD]">
            <ChevronLeft className="size-4 rotate-180" />{enrolled ? "العودة إلى دوراتي" : "العودة إلى الاستكشاف"}
          </Link>

          <section className="rounded-2xl border border-[#E2E0EF] bg-white p-5 sm:p-7" aria-labelledby="course-title">
            <div className="grid gap-7 lg:grid-cols-[1fr_auto] lg:items-end">
              <div>
                <div className="mb-4 flex flex-wrap gap-2">
                  {course.subject_name && <span className="rounded-full bg-[#4F46E5]/5 px-3 py-1 text-xs font-bold text-[#3525CD]">{course.subject_name}</span>}
                  {(gradeName || streamName) && <span className="rounded-full bg-[#F0ECF9] px-3 py-1 text-xs font-bold text-[#464555]">{[gradeName, streamName].filter(Boolean).join(" - ")}</span>}
                </div>
                <h1 id="course-title" className="text-3xl font-black leading-tight text-[#1B1B24] sm:text-4xl">{course.title}</h1>
                {detail.teacher && (
                  <Link href={`/explore/teachers/${detail.teacher.slug}`} className="mt-5 flex w-fit items-center gap-3 rounded-lg focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[#4F46E5]">
              ...
              <div className="w-full lg:w-auto">
                <div className="mb-5 grid grid-cols-3 gap-5 text-center text-xs font-bold text-[#464555] sm:flex sm:justify-end">
                  <span className="grid justify-items-center gap-1"><PlayCircle className="size-5 text-[#4F46E5]" />{course.lesson_count} درس</span>
                  <span className="grid justify-items-center gap-1"><Clock3 className="size-5 text-[#4F46E5]" />{formatDuration(course.total_duration_minutes)}</span>
                  <span className="grid justify-items-center gap-1"><ClipboardList className="size-5 text-[#4F46E5]" />{examCount} اختبار</span>
                </div>
                {enrolled ? (
                  <button type="button" onClick={startCourse} className="inline-flex h-12 w-full cursor-pointer items-center justify-center gap-2 rounded-xl bg-[#4F46E5] px-7 text-sm font-black text-white transition-colors hover:bg-[#3525CD] lg:w-auto">
                    عرض محتوى الكورس<ArrowLeft className="size-4" />
                  </button>
                ) : (
                  <button type="button" onClick={() => void startCheckout()} disabled={checkoutLoading} className="inline-flex h-12 w-full cursor-pointer items-center justify-center gap-2 rounded-xl bg-[#4F46E5] px-7 text-sm font-black text-white transition-colors hover:bg-[#3525CD] disabled:cursor-wait disabled:opacity-70 lg:w-auto">
            ...
            <div className="mt-6 flex flex-wrap items-center gap-x-6 gap-y-2 border-t border-[#E2E0EF] pt-4 text-xs font-bold text-[#777587]">
              {detail.enrollment && <span className="inline-flex items-center gap-2"><CalendarClock className="size-4 text-[#4F46E5]" />الاشتراك متاح حتى {formatExpiry(detail.enrollment.expires_at)}</span>}
              {!!chapters.length && <span className="inline-flex items-center gap-2"><FolderOpen className="size-4 text-[#4F46E5]" />{chapters.length} {chapters.length === 1 ? "وحدة" : "وحدات"}</span>}
            </div>
          </section>
          ...
          {enrolled && activeVideo?.item.bunny_stream_embed_url && (
            <section id="course-player" className="scroll-mt-24 pt-8" aria-labelledby="player-title">
              <div className="overflow-hidden rounded-2xl border border-[#E2E0EF] bg-[#11111A]">
                ...
                <div className="bg-white px-5 py-4">
                  <p className="text-xs font-bold text-[#4F46E5]">{activeVideo.lesson.title}</p>
          ...
          <nav className="mt-8 flex gap-2 overflow-x-auto border-b border-[#E2E0EF]" aria-label="أقسام الكورس">
            {["المحتوى", "الاختبارات", "الملفات", "المناقشات", "التقدم"].map((tab, index) => (
              <button key={tab} type="button" disabled={index !== 0} title={index !== 0 ? `${tab} - قريباً` : undefined} className={`shrink-0 border-b-2 px-4 py-3 text-sm font-black ${index === 0 ? "border-[#4F46E5] text-[#3525CD]" : "cursor-not-allowed border-transparent text-[#A6A3B5]"}`}>{tab}</button>
            ))}
          </nav>

          <section id="course-content" className="scroll-mt-24 pt-8" aria-labelledby="content-title">
            <div className="rounded-2xl border border-[#E2E0EF] bg-white p-5 sm:p-7">
              <div className="mb-6 flex items-center gap-3">
                <span className="flex size-11 items-center justify-center rounded-xl bg-[#F0ECF9] text-[#4F46E5]"><GraduationCap className="size-6" /></span>
                <div><h2 id="content-title" className="text-xl font-black sm:text-2xl">{enrolled ? "محتوى الكورس" : "خطة الكورس"}</h2><p className="mt-0.5 text-xs text-[#777587]">{lessons.length ? `${lessons.length} درس ${enrolled ? "متاح ضمن اشتراكك" : "في هذا الكورس"}` : "سيظهر المحتوى المنشور هنا"}</p></div>
              </div>

              {chapters.some((chapter) => chapter.lessons.length) ? (
                <div className="space-y-8">
                  {chapters.filter((chapter) => chapter.lessons.length).map((chapter, chapterIndex) => (
                    <section key={`${chapter.id}-${chapterIndex}`} aria-labelledby={`chapter-${chapter.id}-${chapterIndex}`}>
                      <h3 id={`chapter-${chapter.id}-${chapterIndex}`} className="mb-2 text-lg font-black text-[#1B1B24] sm:text-xl">{chapter.title || "دروس الكورس"}</h3>
                      <div>
                        {chapter.lessons.map((lesson) => enrolled ? (
                          <LessonRow key={lesson.id} lesson={lesson} expanded={expandedLessonId === lesson.id} activeVideoId={activeVideo?.item.id ?? null} onToggle={() => setExpandedLessonId((current) => current === lesson.id ? null : lesson.id)} onPlay={playVideo} />
                        ) : (
                          <div key={lesson.id} className="flex min-h-16 items-center gap-3 border-b border-[#E2E0EF] px-2 py-3 last:border-b-0">
                            <span className="flex size-9 shrink-0 items-center justify-center rounded-lg bg-[#F0ECF9] text-[#4F46E5]"><LockKeyhole className="size-4" /></span>
                            <span className="min-w-0 flex-1"><strong className="block truncate text-sm">{lesson.title}</strong><span className="text-xs text-[#777587]">{lesson.items.length} عناصر تعليمية</span></span>
                            <span className="text-[11px] font-bold text-[#777587]">يتطلب الاشتراك</span>
                          </div>
                        ))}
                      </div>
                    </section>
                ...
                <div className="flex min-h-64 flex-col items-center justify-center border-y border-[#E2E0EF] px-4 text-center">
