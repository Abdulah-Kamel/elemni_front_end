...
function StudentIdentity({ user }: { user: UserDto | null }) {
  ...
  return (
    <div className="flex items-center gap-2.5">
      <span className="flex size-9 items-center justify-center rounded-full bg-[#E2DFFF] text-xs font-black text-[#3525CD]">{initials}</span>
  ...
  return (
    <>
      <Link href="/dashboard" onClick={close} className="mx-5 mb-8 flex items-center gap-3">
        <span className="flex size-11 items-center justify-center rounded-xl bg-[#4F46E5] text-white"><GraduationCap className="size-6" /></span>
        <span><strong className="block text-2xl font-black text-[#3525CD]">علمني</strong><span className="text-xs text-[#777587]">منصة التعليم الذكي</span></span>
      </Link>

      <nav aria-label="بوابة الطالب" className="space-y-1">
        {enabledNav.map(({ label, href, icon: Icon, id }) => (
          <Link key={id} href={href} onClick={close} className={`flex h-12 items-center gap-3 border-s-4 px-6 text-sm font-bold transition ${active === id ? "border-[#4F46E5] bg-[#4F46E5]/5 text-[#3525CD]" : "border-transparent text-[#464555] hover:bg-[#F5F2FF]"}`}>
...
export default function StudentPortalShell({ children, user, active = "dashboard" }: { children: ReactNode; user: UserDto | null; active?: string }) {
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-[#F9F8FC] font-cairo text-[#1B1B24]">
      ...
      <header className="student-portal-header fixed top-0 z-30 flex h-16 items-center justify-between border-b border-[#E2E0EF] bg-white/95 px-4 backdrop-blur-md md:px-8">
        <div className="flex items-center gap-2">
          <button onClick={() => setMobileOpen(true)} aria-label="فتح قائمة بوابة الطالب" className="flex size-10 cursor-pointer items-center justify-center rounded-lg text-[#464555] hover:bg-[#F5F2FF] md:hidden"><Menu className="size-6" /></button>
          <Link href="/dashboard" className="flex items-center gap-2 font-black text-[#3525CD] md:hidden"><GraduationCap className="size-5" />علمني</Link>
        </div>
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          <aside className="student-portal-drawer absolute inset-y-0 flex w-[min(82vw,19rem)] flex-col bg-white py-6 shadow-2xl">
            <button onClick={() => setMobileOpen(false)} aria-label="إغلاق القائمة" className="student-portal-drawer-close absolute top-4 flex size-9 cursor-pointer items-center justify-center rounded-lg text-[#777587] hover:bg-[#F5F2FF]"><X className="size-5" /></button>
