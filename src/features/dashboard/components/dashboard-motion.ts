...
export const portalNestedContainerVariants: Variants = {
  hidden: {},
  show: { transition: { staggerChildren: 0.05 } },
};

export const portalNestedItemVariants: Variants = {
...
export function scrollIntoViewById(id: string, options?: ScrollIntoViewOptions) {
  ...
  document.getElementById(id)?.scrollIntoView({ ...options, behavior: reduceMotion ? "auto" : (options?.behavior ?? "smooth") });
}
