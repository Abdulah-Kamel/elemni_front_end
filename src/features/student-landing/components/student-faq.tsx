...
export function StudentFaqSection() {
  const t = useTranslations("studentLanding.faq");
  const [openIndex, setOpenIndex] = useState<number | null>(0);
  const baseId = useId();

  return (
    <section id="faq" className="bg-brand-50 py-16 md:py-24">
      <div className="mx-auto max-w-3xl px-4">
        <Reveal>
          <h2 className="text-gradient text-center text-3xl font-black md:text-4xl">
            {t("heading")}
          </h2>
          ...
          {FAQ_KEYS.map((key, i) => {
            ...
            return (
              <Reveal key={key} delay={i * 60}>
                <div className="rounded-2xl bg-white px-5 border border-brand-200/70">
                  <h3>
                    <button
                      type="button"
                      id={buttonId}
