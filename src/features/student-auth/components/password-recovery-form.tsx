      ...
      <AuthCard>
        ...
        {sent ? (
          <div className="text-center">
            ...
            <h2 className="mt-4 text-lg font-black">راجع بريدك الإلكتروني</h2>
            <p className="mt-2 text-sm leading-6 text-[#777587]">
          ...
          <form onSubmit={submit} className="space-y-4">
            <div>
              <label htmlFor="email" className="mb-1.5 block text-xs font-bold text-[#464555]">البريد الإلكتروني</label>
              <div className="relative">
                <Mail className="pointer-events-none absolute end-3.5 top-1/2 size-4.5 -translate-y-1/2 text-[#777587]" />
        ...
        {complete ? (
          ...
          <>
            <div className="mb-7 text-center">
              ...
              <p className="mt-2 text-sm text-[#777587]">اختر كلمة قوية لا تقل عن 8 أحرف.</p>
            </div>
            {!token ? (
              <div role="alert" className="rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm font-bold text-red-700">رابط الاستعادة غير مكتمل.</div>
            ) : (
              <form onSubmit={submit} className="space-y-4">
                {[{ id: "password", label: "كلمة المرور الجديدة" }, { id: "password_confirmation", label: "تأكيد كلمة المرور" }].map((field) => (
                  <div key={field.id}>
                    <label htmlFor={field.id} className="mb-1.5 block text-xs font-bold text-[#464555]">{field.label}</label>
                    <div className="relative">
