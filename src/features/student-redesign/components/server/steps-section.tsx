  ...
  return (
    <Section id="how">
      ...
      <div className="relative grid gap-8 md:grid-cols-3">
        <div
          aria-hidden
          className="steps-connector absolute inset-x-[16%] top-6 hidden origin-right border-t-2 border-dashed border-amber-400/60 md:block"
        />
        {steps.map((step, i) => (
          <Reveal key={step.num} delay={i * 100} className="relative">
            <div className="flex flex-col items-center text-center transition-transform duration-300 hover:-translate-y-1.5">
              <div
                className={cn("steps-number relative z-10 grid size-12 place-items-center rounded-full text-lg font-black text-white shadow-lg", step.circleClass)}
              >
                {step.num}
              </div>
