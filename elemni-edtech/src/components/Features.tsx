  ...
  return (
    <section id="features" className="py-20 bg-white dark:bg-[#0B132B] relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        {/* 3-Step Student Journey Grid */}
        <div className="relative">
          {/* Subtle Connecting Dashed Line for Desktop (RTL Flow) */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 lg:gap-12 relative z-10">
            {steps.map((step, index) => {
              ...
              return (
                <div
                  key={step.id}
                  className="group relative bg-sky-50/40 dark:bg-slate-800/40 hover:bg-white dark:hover:bg-slate-800/90 rounded-3xl p-8 lg:p-10 text-center flex flex-col items-center transition-all duration-300 hover:shadow-xl hover:-translate-y-1.5 border border-sky-100/70 dark:border-slate-700/50"
                >
                  ...
                  {/* Icon Container - Bold Duotone Style */}
                  ...
                  {/* Title */}
                  <h3 className="text-2xl font-black text-slate-900 dark:text-white mb-3 font-cairo group-hover:text-primary transition-colors">
                    {step.title}
                  </h3>

                  {/* Single Description Sentence */}
                  <p className="text-sm sm:text-base text-slate-600 dark:text-slate-300 leading-relaxed font-medium max-w-xs">
                    {step.description}
                  </p>

                  {/* Desktop Connecting Arrow between steps 1->2 and 2->3 */}
