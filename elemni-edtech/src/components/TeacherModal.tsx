...
export const TeacherModal: React.FC<TeacherModalProps> = ({ teacher, onClose, onBook, onViewFullProfile }) => {
      ...
      <div className="bg-white rounded-3xl max-w-2xl w-full p-6 sm:p-8 shadow-2xl relative border border-slate-100 text-right my-8">
        ...
        <div className="flex flex-col sm:flex-row items-center sm:items-start gap-5 pb-6 border-b border-slate-100">
          ...
          <div className="space-y-2 text-center sm:text-right flex-1">
            ...
            <h3 className="text-2xl font-black text-[#0F172A] font-cairo">
              {teacher.name}
            </h3>
            ...
            <div className="flex flex-wrap items-center justify-center sm:justify-start gap-1.5 pt-0.5">
              {(teacher.subjects && teacher.subjects.length > 0 ? teacher.subjects : [teacher.subject]).map((sub, idx) => (
                <span key={idx} className="bg-sky-50 text-primary border border-sky-200/60 font-extrabold text-[11px] px-2.5 py-0.5 rounded-full">
                  {sub}
                </span>
              ))}
            </div>
        ...
        <div className="space-y-4 mb-6">
          ...
          <div className="flex flex-wrap gap-2">
            {teacher.specialties.map((spec, idx) => (
              <span
                key={idx}
                className="bg-primary-light text-primary font-bold text-xs px-3 py-1.5 rounded-xl flex items-center gap-1.5"
              >
        ...
        {/* Teacher's Individual Courses List */}
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            ...
            {onViewFullProfile && (
              <button
                ...
                className="text-xs font-black text-primary hover:text-primary-hover bg-sky-50 px-3 py-1.5 rounded-xl border border-sky-200 transition-all flex items-center gap-1.5 self-start sm:self-auto cursor-pointer"
              >
                <Sparkles className="w-3.5 h-3.5 text-amber-500" />
                <span>عرض الصفحة الشخصية الكاملة</span>
              </button>
            )}
          </div>
          ...
          <div className="space-y-3">
            {teacher.courses && teacher.courses.length > 0 ? (
              teacher.courses.map((course) => (
                <div
                  key={course.id}
                  className="p-4 rounded-2xl bg-[#F8FAFC] border border-slate-200 hover:border-primary transition-all flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4"
                >
                  <div className="space-y-1 flex-1">
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-bold text-primary bg-primary-light px-2.5 py-0.5 rounded-md">
                        {course.duration}
                      </span>
                      <span className="text-xs text-slate-500 font-medium">
                        {course.sessionsCount} حصص مباشرة
                      </span>
                    </div>
        ...
        <div className="space-y-2 mb-6 pt-2 border-t border-slate-100">
          ...
          <div className="flex flex-wrap gap-2 text-xs text-slate-600">
            {teacher.schedule.map((slot, idx) => (
              <span key={idx} className="bg-slate-100 px-3 py-1 rounded-lg font-medium">
                {slot}
              </span>
            ))}
          </div>
