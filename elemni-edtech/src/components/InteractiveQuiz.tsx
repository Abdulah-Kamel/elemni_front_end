...
import { HelpCircle, CheckCircle, XCircle, RotateCcw, Award, Sparkles, ArrowLeft } from 'lucide-react';

interface InteractiveQuizProps {
  onExploreTeachers: () => void;
}

export const InteractiveQuiz: React.FC<InteractiveQuizProps> = ({ onExploreTeachers }) => {
  ...
  return (
    <section id="quiz" className="py-20 bg-[#F8FAFC] relative">
            ...
            <div>
              ...
              <div className="flex items-center justify-between pb-4 border-b border-slate-100 mb-6">
                <div className="flex items-center gap-2">
                  <span className="bg-primary-light text-primary font-extrabold text-xs px-3 py-1 rounded-full">
                    مادة: {currentQ.subject}
                  </span>
              ...
              {/* Question Text */}
              <h3 className="text-lg sm:text-xl font-black text-[#0F172A] leading-relaxed mb-6 text-right font-cairo">
                {currentQ.question}
              </h3>
                ...
                {currentQ.options.map((option, idx) => {
                  ...
                  return (
                    <button
                      ...
                      disabled={isAnswered}
                      className={`w-full p-4 text-right rounded-2xl border text-sm transition-all cursor-pointer flex items-center justify-between gap-3 ${buttonStyle}`}
                    >
                ...
                {!isAnswered ? (
                  <button
                    ...
                    className={`px-6 py-3 rounded-xl font-bold text-sm text-white transition-all ${
                      selectedOption !== null
                        ? 'bg-primary hover:bg-primary-hover shadow-md cursor-pointer'
            ...
            <div className="text-center py-6 space-y-6">
              ...
              <div className="space-y-2">
                <h3 className="text-2xl font-black text-[#0F172A]">
                  ممتاز! أكملت التقييم بنجاح 🎉
                </h3>
                <p className="text-base font-bold text-primary">
                  حصلت على {score} من {QUIZ_QUESTIONS.length} إجابات صحيحة
                </p>
              ...
              <div className="flex flex-wrap items-center justify-center gap-4 pt-4">
                <button
                  ...
                  className="px-6 py-3 bg-primary hover:bg-primary-hover text-white font-bold text-sm rounded-xl shadow-md transition-all cursor-pointer"
                >
                ...
                <button
                  ...
                  className="px-5 py-3 bg-slate-100 text-slate-800 font-bold text-sm rounded-xl hover:bg-slate-200 transition-all cursor-pointer flex items-center gap-2"
                >
